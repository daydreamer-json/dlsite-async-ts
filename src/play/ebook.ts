...
export class EbookSession implements AsyncDisposable {
  ...
  private pages(): MetaRecord[] {
    const value = this.#meta["pages"];
    return Array.isArray(value) ? (value as MetaRecord[]) : [];
  }
  ...
  private async fetchDownloadToken(): Promise<ViewerToken> {
    ...
    const data = (await response.json()) as MetaRecord;
  ...
  private async fetchMeta(): Promise<MetaRecord> {
    ...
    return (await response.json()) as MetaRecord;
  }
  ...
  async downloadPage(
    ...
    if (wantImage) {
      ...
      try {
        ...
        console.warn(
          "Image conversion requires installation with sharp (`npm install sharp`)",
        );
        convert = undefined;
      }
        ...
        try {
          ...
          const formatted =
            convert === "jpg"
              ? pipeline.jpeg({
                  quality: 95,
                  ...((options.saveOptions ?? {}) as object),
                })
              : pipeline.png((options.saveOptions ?? {}) as object);
