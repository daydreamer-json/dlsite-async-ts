...
function clean(content: string | undefined | null): string {
  return (content ?? "").normalize("NFKC").trim();
}

type FieldName = Extract<keyof WorkFields, string>;

type CellValue = string | string[] | number | DateTime;

interface RowParser {
  field: FieldName;
  headers: ReadonlySet<string>;
  parse(td: Element, $: CheerioAPI): CellValue;
}

function textParser(field: FieldName, headers: readonly string[]): RowParser {
  return {
    field,
...
function dateParser(field: FieldName, headers: readonly string[]): RowParser {
  return {
    field,
    headers: new Set(headers),
    parse(td, $) {
...
function intParser(field: FieldName, headers: readonly string[]): RowParser {
  return {
    field,
    headers: new Set(headers),
    parse(td, $) {
...
function makerParser(field: FieldName, headers: readonly string[]): RowParser {
  return {
    field,
    headers: new Set(headers),
    parse(td, $) {
      ...
      const name =
        span.length > 0 ? clean(span.text()) : clean($(td).text());
      if (name === "") {
...
function listParser(field: FieldName, headers: readonly string[]): RowParser {
  return {
    field,
    headers: new Set(headers),
    parse(td, $) {
...
const PARSERS: readonly RowParser[] = [
...
function findParser(header: string): RowParser | undefined {
  return PARSERS.find((parser) => parser.headers.has(header));
}

function assignValue(
  info: Partial<WorkFields>,
  field: FieldName,
  value: CellValue,
): void {
  // Each parser's field key matches its produced value type.
  (info as Record<FieldName, unknown>)[field] = value;
}

function tryParseCell(
  info: Partial<WorkFields>,
  parser: RowParser,
  td: Element,
  $: CheerioAPI,
...
function parseComipoProductInfo(
  info: Partial<WorkFields>,
  dl: Element,
  $: CheerioAPI,
): void {
...
export function parseWorkHtml(content: string): Partial<WorkFields> {
  const $ = cheerio.load(content);
  const info: Partial<WorkFields> = {};
...
/** Parse circle HTML. */
export function parseCircleHtml(content: string): CircleHtmlInfo {
  const $ = cheerio.load(content);
...
export function parseLoginToken(content: string): string {
  const $ = cheerio.load(content);
