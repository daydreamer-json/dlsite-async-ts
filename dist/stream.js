...
export function xorBytes(data, key, offset = 0) {
...
export function xorTransform(key) {
 ...
 */
export async function streamToFile(response, destPath, transform) {
    const tempPath = join(dirname(destPath), `.${basename(destPath)}.${crypto.randomUUID()}.tmp`);
