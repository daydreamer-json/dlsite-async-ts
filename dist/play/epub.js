...
import { mkdir, mkdtemp, open, readFile, rename, rm, unlink, utimes, } from "node:fs/promises";
...
import { CsrReflowableToken, CsrToken, ZipTree, } from "./models.js";
...
function preprocessSettingsFromJson(data) {
...
export function loadFaceXml(content) {
...
export function loadPageXml(content) {
    const $ = cheerio.load(content, { xmlMode: true });
...
export class EpubFixedSession {
    ...
    async fetchDownloadToken() {
        ...
        return CsrToken.fromJson(values);
    }
    /**
    ...
    async downloadPage(index, destDir, options = {}) {
        ...
        for (const part of pageInfo.parts) {
            ...
            const filename = pageInfo.parts.length === 1 ? `${indexStem}.jpg` : `${partStem}.jpg`;
            ...
            if (options.descramble === true &&
                part.scramble &&
                this.scrambleSize !== undefined) {
                await descrambleFixedLayout(dest, this.scrambleSize, pageInfo.scramble, options.saveOptions ?? {});
            }
            results.push(dest);
        }
        return results;
    }
    async fetchPageInfo(index) {
        const token = this.token;
...
export async function descrambleFixedLayout(path, scrambleSize, scramble, saveOptions = {}) {
    let sharp;
    try {
    ...
    const formatted = extension === "jpg" || extension === "jpeg"
...
export class EpubReflowableSession {
    constructor(playApi, ziptree, playfile, workno) {
        this.deobfuscators = new Map();
        this.play = playApi;
        this.ziptree = ziptree;
        this.playfile = playfile;
        const resolvedWorkno = workno ?? ziptree.workno;
     ...
     *   FileExistsError: destination file already exists.
     */
    async downloadEpub(destDir, options = {}) {
        const token = this.token;
        if (token === undefined) {
        ...
        try {
            ...
            const settingsEntry = await this.fetchEpubEntry(tempDir, "preprocess-settings.json", token);
            const settings = preprocessSettingsFromJson(JSON.parse(await readFile(settingsEntry.path, "utf8")));
            const container = await this.fetchEpubEntry(tempDir, "META-INF/container.xml", token);
            addEntry(container);
    ...
    async fetchEpubEntry(tempDir, epubEntry, token) {
        ...
        await streamToFile(response, dest, (chunk, offset) => this.decrypt(chunk, offset));
        let mtime;
    ...
    async fetchEpubContents(opf, settings, token, tempDir) {
        ...
        $("manifest > item").each((_i, item) => {
            ...
            tasks.push(semaphore.run(async () => {
                try {
    ...
    deobfuscate(path, settings, mediaType) {
    ...
    async deobfuscateImage(path, settings, mediaType) {
    ...
    async deobfuscateText(path, settings, mediaType) {
        ...
        $("span").each((_i, span) => {
            ...
            deobfuscateElement((s) => deobfuscator.decode(s), span);
        });
        await atomicWriteBytes(path, Buffer.from($.xml(), "utf8"));
    }
}
function deobfuscateElement(decode, element) {
...
async function getRootfile(containerPath) {
    const $ = cheerio.load(await readFile(containerPath, "utf8"), {
        xmlMode: true,
    });
    const rootfile = $("rootfile").first();
    const fullPath = rootfile.attr("full-path") ?? $("*|rootfile").first().attr("full-path");
...
async function atomicWriteBytes(path, data) {
    ...
    const tempPath = posix.join(parent, `.${posix.basename(path)}.${crypto.randomUUID()}.tmp`);
    const { writeFile } = await import("node:fs/promises");
    await writeFile(tempPath, data);
    await rename(tempPath, path);
}
async function writeZipArchive(dest, entries) {
    const tempPath = posix.join(posix.dirname(dest), `.${posix.basename(dest)}.${crypto.randomUUID()}.tmp`);
...
export class Deobfuscator {
    static { this.HIRAGANA = "あいうえおかがきぎくぐけげこごさざしじすずせぜそぞただちぢつづてでとどなにぬねのはばぱひびぴふぶぷへべぺほぼぽまみむめもやゆよらりるれろわゐゑをんゔ"; }
    static { this.KATAKANA = "アイウエオカガキギクグケゲコゴサザシジスズセゼソゾタダチヂツヅテデトドナニヌネノハバパヒビピフブプヘベペホボポマミムメモヤユヨラリルレロワヰヱヲンヴヷヸヹヺ"; }
    static { this.KANJI = "国人大年生地的日化本会自中一民政分世業者合動法発行方立権間定力成主子出代物体社" +
    ...
    constructor(seed) {
        ...
        if (cached === undefined) {
            ...
            const shuffled = Deobfuscator.shuffle(hiragana, xorshift) +
